...
const NavBar = () => {
  ...
  const navLinks = [
    ...
    ...(festivalActive ? [{ name: "Dec Festival", path: "/december-festival", highlight: true }] : []),
          ...
          {navLinks.map((link) => (
            link.highlight ? (
              <Link
                key={link.path}
                to={link.path}
                className={`px-3 py-1 rounded-full text-xs font-heading font-extrabold uppercase tracking-wider transition-all duration-300 flex items-center gap-1.5 ${
                  isActive(link.path)
                    ? "bg-volt-400 text-darkslate-950 shadow-md font-black"
                    : "bg-volt-400/15 text-volt-300 border border-volt-400/40 hover:bg-volt-400 hover:text-darkslate-950"
                }`}
              >
        ...
        {navLinks.map((link, index) => (
          <Link
            ...
            className={`font-heading font-bold text-2xl tracking-wider transition-all duration-300 ${
              link.highlight
                ? "text-volt-300 border border-volt-400/50 bg-volt-400/10 px-5 py-1.5 rounded-full scale-105"
                : isActive(link.path)
                ? "text-volt-400 scale-105"
